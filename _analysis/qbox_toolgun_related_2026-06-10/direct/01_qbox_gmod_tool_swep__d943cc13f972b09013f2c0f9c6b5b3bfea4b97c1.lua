...
SWEP.ShootSound			= "qboxsounds/toolgun.wav"
resource.AddFile( 'sound/'..SWEP.ShootSound )
SWEP.ShootSound	= Sound( SWEP.ShootSound	)
SWEP.Tool = {}

if SERVER then
	util.AddNetworkString'SToolDecal'
end
...
function SWEP:DoShootEffect( hitpos, hitnormal, entity, physbone, bFirstTimePredicted )
	...
	if SERVER then
		net.Start("SToolDecal", true) -- QBox
			net.WriteEntity(self)
			net.WriteEntity(self:GetOwner())
			net.WriteString(tostring(self.Mode))
			net.WriteAngle(ang)
		net.Broadcast()
	end
end

net.Receive("SToolDecal",function()
	local ent =   net.ReadEntity()
	local owner = net.ReadEntity()
	local mode =  net.ReadString()
	local ang =   net.ReadAngle()
	if not owner:IsValid() then return end
	hook.Run("SToolDecal", ent,owner,mode,ang)
end)
...
function SWEP:SecondaryAttack()
	...
	tr.mask = bit.bor( CONTENTS_SOLID, bit.bxor(CONTENTS_WATER,bit.band(util.PointContents(tr.start),CONTENTS_WATER)), CONTENTS_MOVEABLE, CONTENTS_MONSTER, CONTENTS_WINDOW, CONTENTS_DEBRIS, CONTENTS_GRATE, CONTENTS_AUX )
	local trace = util.TraceLine( tr )
	if ( !trace.Hit ) then return end
	...
	-- Does the server setting say it's ok?
	if ( !tool:Allowed() ) then return end
	if ( !gamemode.Call( "CanTool", self:GetOwner(), trace, mode ) ) then return end
